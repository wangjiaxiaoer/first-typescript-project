      ...
      <div class="collapse navbar-collapse navbar-right">
        <ul class="nav navbar-nav">
          <li class="add dropdown">
            <a class="create-new dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">新建</a>
  ...
  doFilter(cid: number): number {
    if (cid == -1) {
      ...
      return this.$store.state.aHelper.memoList.filter((ele: any) => {
        return cid === ele.categoryId
      }).length
    }
  }
  filterList(num: number): void {
    this.$store.state.filterCateId = num
  }
}
</script>
<style scoped>
